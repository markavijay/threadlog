      ...
      <div style="padding:0 16px 32px">
        ...
        ${!TL_DB.isFileSystemSupported() ? `
          ...
          <div style="display:flex;align-items:center;gap:10px;padding:12px 0;border-bottom:1px solid var(--border)">
            ...
            <button onclick="TL_DB.connectFolder().then(()=>{TL_APP.toast('Folder reconnected'); TL_CONTACTS.render(); TL_PROJECTS.render(); TL_RECONNECT.render(); TL_SHEETS.openSettings()}).catch(e=>TL_APP.toast(e.message))" style="font-size:12px;color:var(--tl-accent);background:none;border:none;cursor:pointer;font-family:var(--font)">Change</button>
          </div>` : `
          <div style="padding:12px 0;border-bottom:1px solid var(--border)">
            ...
            <button onclick="TL_DB.connectFolder().then(()=>{TL_APP.toast('Folder connected ✓'); TL_CONTACTS.render(); TL_PROJECTS.render(); TL_RECONNECT.render(); TL_SHEETS.openSettings()}).catch(e=>TL_APP.toast(e.message))" style="display:flex;align-items:center;justify-content:center;gap:10px;width:100%;padding:11px;border:1px solid var(--border);border-radius:var(--radius-lg);background:var(--bg);font-size:14px;font-weight:500;color:var(--text-primary);font-family:var(--font);cursor:pointer">
